...
from dataDealer.kaldi_tester import KaldiTester
...
class Scorer:
    ...
    def __init__(self, model_path, trials_path, scp_path,
                 ...
                 mgr=None, processes=None, check=False, model=None):
    ...
    def get_scores(self, start, end):
        ...
        try:
            specified_score = getattr(model, 'score')
        except:
            specified_score = False
            pass
        ...
        with torch.no_grad():
            for cnt, (mat1, mat2, utt1, utt2) in enumerate(dataloader):
                ...
                mat2 = mat2.float()
                if specified_score:
                    scores = model.score(mat1, mat2).squeeze()
                else:
...
if __name__ == '__main__':
    ...
    args = main()
    conf_path = '../../nnet/config.ini'
    parser = ConfigParser()
    parser.read(conf_path)
    configs = {sec: {opt: parser[sec][opt] for opt in parser.options(sec)} for sec in parser.sections()}
    ...
    for epoch in args.epochs:
        ...
        scorer = Scorer(model_path=model_path,
                        scp_path=configs['test']['scp_path'] if args.scp_path is None else args.scp_path,
        ...
        print('%s/%s' % (basename, configs['exp']['name']), file=fr)
